...
int main(int argc, char **argv)
{
    ...
    if (!retVal) {
        ...
        cout << "Usage: " << argv[0] << " -impl <value> {0:'Global Memory', 1:'Texture', 2:'Pitch', 3:'Shared Memory', 4:'2D Grid'}" << endl; return 1;
    }
    ...
    if (!suppressOut) cout << "image dimensions: " << w << " x " << h << endl;
    ...
    switch(impl) {
        case 0:
            stereo_projection_PD(imgInLeft, imgInRight, imgOutDepth, dim3(w, h, 0), nc, dim3(w, h, nt), steps, MU, SIGMA, TAU); break;
        case 1:
            stereo_projection_PD_tex(imgInLeft, imgInRight, imgOutDepth, dim3(w, h, 0), nc, dim3(w, h, nt), steps, MU, SIGMA, TAU); break;
        case 2:
            stereo_projection_PD_pitch(imgInLeft, imgInRight, imgOutDepth, dim3(w, h, 0), nc, dim3(w, h, nt), steps, MU, SIGMA, TAU); break;
        case 3:
            stereo_projection_PD_sm(imgInLeft, imgInRight, imgOutDepth, dim3(w, h, 0), nc, dim3(w, h, nt), steps, MU, SIGMA, TAU); break;
        default:
            break;
    }
    
    timer.end();  float t = timer.get();  // elapsed time in seconds
    if(!suppressOut) cout << "time: " << t*1000 << " ms" << endl;
    ...
    if(!suppressOut) showImage("Input Left", mInLeft, 100, 100);
    // show at position (x_from_left=100,y_from_above=100)  
    if(!suppressOut) showImage("Input Right", mInRight, 100+w+40, 100);

    // ### Display your own output images here as needed
    if(!suppressOut) showImage("Depth Mapping", mOutDepth, 100+2*w+40, 100);
