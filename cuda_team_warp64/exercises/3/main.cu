...
int main(int argc, char **argv)
{
    ...
    // GPU computation
    if(hardware == "GPU") {
        cout << "Running invert image on GPU" << endl;
        invert_image_caller(imgIn, imgOut, w, h, nc);
    }
    // CPU computation
    else if(hardware == "CPU") {
        cout << "Running invert image on CPU" << endl;
        invert_image_CPU(imgIn, imgOut, w, h, nc);
    }
    // invalid hardware input by user
    else {
    ...
    // "imwrite" assumes channel range [0,255]
    cv::imwrite("./images/out/result.png", mOut*255.f);
